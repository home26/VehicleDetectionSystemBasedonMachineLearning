...
def draw_boxes(image, out_scores, out_boxes, out_classes, class_names, colors):
    ...
    class_length = len(out_classes)
    class_length = str(class_length)
    print('the class length:',class_length)
    print('the type of class length:',type(class_length))
    with open('number.txt','w') as f:
        f.write(class_length)
...
def predict(sess, in_file,out_file):
    ...
    print('Detection:','1:',out_scores,'2:',out_boxes,'3:',out_classes)
    ### END CODE HERE ###
...
image_shape = (400., 600.)#输入图片大小 (400., 600.) 视频每帧大小
...
def testmainvideo(VIDEO_PATH,EXTRACT_FOLDER,EXTRACT_FREQUENCY,FPS,SIZE):
    ...
    videoWriter = cv2.VideoWriter(EXTRACT_FOLDER, cv2.VideoWriter_fourcc(*'MJPG'), FPS, SIZE)
    #'oto_other.avi'
